...
const App = () => {
  ...
  const [flareData, setFlareData] = useState(makeDummy());
  const [dark, setDark] = useState(true); // start in dark-mode

  useEffect(() => setFlareData(makeDummy()), [day]);
  ...
  return (
    ...
    >
      ...
      <section className="mb-6">
        {/* desktop */}
        <div
          className={`hidden sm:flex overflow-x-auto gap-3 ${panel} rounded-xl p-3`}
        >
          {flareData.map((h, i) => (
        ...
        >
          {flareData.map((h, i) => (
            <div
              key={i}
              className="flex justify-between rounded-lg px-3 py-1 bg-white/20 sm:bg-transparent"
            >
