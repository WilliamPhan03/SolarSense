...
SAVE = "data/processed/hourly_goes_1day_clean.csv"
...
def fetch_training_data(save_path: str = SAVE) -> None:
    ...
    # Only take each hour
    long_df['time_tag'] = pd.to_datetime(long_df['time_tag'], utc=True)
    long_df = long_df[long_df['time_tag'].dt.minute == 0].copy()

    # Add the "goes_class" column
    long_df["goes_class"] = long_df["long_flux"].apply(flux_to_class)

    long_df["timestamp"] = (
        long_df["time_tag"].dt.strftime("%Y-%m-%d %H:%M:%S")
    )
